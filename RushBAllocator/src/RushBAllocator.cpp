...
size_t RushBAllocator::Allocate(const size_t size){
    ...
    if (m_freeList[index]) {
        ...
        offset += blockSize;
#ifdef _DEBUG
        std::cout << "A" << "\t@H " << (void*) headerAddress << "\tD@ " <<(void*) dataAddress << "\tS " << ((RushBAllocator::AllocationHeader *) headerAddress)->blockSize <<  "\tAP " << alignmentPadding << "\tP " << padding << "\tM " << offset  << std::endl;
#endif
...
void RushBAllocator::Free(void* ptr, size_t size){
    int32_t index = blockSizeLookup[size + AllocationHeaderSize];
